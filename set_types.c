...
void	set_types(t_token **list)
{
	...
	while(tmp && tmp->next)
	{
		// here is some edits by me to set the types of the nodes after the signs
			// if the sign is one sided we set the next node to infile or outfile
			// if the sign is two sided we set the next node to cmd
		if(is_one_sided(tmp))
		{
			if(tmp->type == TOK_REDIR_IN || tmp->type == TOK_HEREDOC)
